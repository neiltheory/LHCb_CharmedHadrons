...
// Prints variables to plot .
// Prints min and max values of mass (GeV), Chi Squared and FCN value to screen.
...
void fitfunc() {
  ...
  */
  ...
  ds->RooAbsData::getRange(Lambda_cplus_M, lowest, highest);
   ...
   // Fit model
   model.fitTo(*ds, Extended()); //?!?! Does the same
   //model.fitTo(*ds) ; //?!?! Does the same
   //RooFitResult* rmodel = model.fitTo(*ds,Save()) ; //<-most recent
   ...
   ds->statOn(Lambda_cplus_Mframe, Layout(0.65,0.88,0.2), What("N")) ; //NB Layout(xmin,xmax,ymax) 
   ...
   Double_t chi2 = Lambda_cplus_Mframe->chiSquare("Model","data",7) ;
   //t1->Draw();
   ...
   c1->SaveAs("extendedLikelihoodFit3.eps");
